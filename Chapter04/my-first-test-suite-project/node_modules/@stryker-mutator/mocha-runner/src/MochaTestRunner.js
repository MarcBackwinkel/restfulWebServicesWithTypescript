...
class MochaTestRunner {
    constructor(log, allFileNames, options) {
        ...
        this.mochaOptions = options.mochaOptions;
    ...
    legacyDiscoverFiles() {
        ...
        if (fileNames.length) {
            ...
            throw new Error(`[${MochaTestRunner.name}] No files discovered (tried pattern(s) ${JSON.stringify(globPatterns, null, 2)}). Please specify the files (glob patterns) containing your tests in ${util_1.propertyPath('mochaOptions', 'spec')} in your config file.`);
        }
        return fileNames;
    }
    ...
    run({ testHooks }) {
        return new Promise((resolve, reject) => {
            try {
                this.purgeFiles();
                const mocha = new LibWrapper_1.default.Mocha({ reporter: StrykerMochaReporter_1.StrykerMochaReporter, bail: true, rootHooks: this.rootHooks });
    ...
    async additionalRequires() {
        if (this.mochaOptions.require) {
            if (LibWrapper_1.default.handleRequires) {
                const rawRootHooks = await LibWrapper_1.default.handleRequires(this.mochaOptions.require);
                if (rawRootHooks) {
                    this.rootHooks = await LibWrapper_1.default.loadRootHooks(rawRootHooks);
                }
            }
            else {
